...
MainWindow::MainWindow(QWidget *parent) :
    ...
    size_t arg_count = QCoreApplication::arguments().size();
...
void MainWindow::UpdateRouteProfile()
    ...
    m_ui->actionDrive->setChecked(p == TExtendedRouteProfileType::EDrive);
    m_ui->actionCycle->setChecked(p == TExtendedRouteProfileType::ECycle);
    m_ui->actionWalk->setChecked(p == TExtendedRouteProfileType::EWalk);
    m_ui->actionHike->setChecked(p == TExtendedRouteProfileType::EHike);
    m_ui->actionCustom_Profile->setChecked(p == TExtendedRouteProfileType::ECustom);
    }
...
void MainWindow::on_actionEdit_Custom_Style_Sheet_triggered()
    {
    if (m_map_form)
        m_map_form->EditCustomStyleSheet();
    }
...
void MainWindow::UpdateCustomRouteProfile()
    ...
    QString s { QString::fromUtf8((const char*)output.Data(),output.Length()) };
